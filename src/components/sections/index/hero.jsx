	...
	return (
		<Section classProp={`${hero.section}`}>
			<Container spacing={'VerticalXXXL'}>
				<TypeAnimation
					className={`${hero.preHeader} ${space(['verticalXXXLrg'])}`} 
					sequence={[
				...
				<section>
					...
					<h1 className={`${hero.header} ${hero.primaryDim}`} style={{ fontSize: '2.1rem' }}>
  {content.header.usp}
</h1>

				</section>
				<section>
					<p className={`${hero.primaryBright} subtitle ${space(['verticalLrg'])}`}>{/* Add vertical spacing */}
						{content.paragraph}
					</p>
				</section>
				<section style={{ position: 'relative' }}>
					...
					<button
						...
						onClick={() => window.open('https://www.linkedin.com/in/pawanya-kariyawasam-193984230/', '_blank')}
